...
Uses Classes,ComObj,sysutils,Windows,Dialogs,SOAPHTTPClient,Rio, SAQ300007,
     TableCodeService, SecurityService;
...
//      Function GetLevelAccess:String;
      ...
      Procedure HTTPRIO1_BeforeExecute(const MethodName: string; var SOAPRequest: WideString);
      procedure HTTPRIO2_BeforeExecute(const MethodName: String; var SOAPRequest: WideString);
    Public
      Constructor Create;
      ...
      Property MimsPassword:   String Read FMIMSPassword;
      ...
      Property Enviroment:     String Read FEnviroment;
      ...
      //Property LevelAccess:String Read GetLevelAccess;
    End;
...
implementation
Uses Forms, MCTypes, MCUtils, SAQ300005, StdCtrls;
...
{Function TMIMSXServer.GetLevelAccess: String;
Var
...
{   MIMSBlock:           TMIMSBlock;
   ...
   ApplicationName:     String;}
{   HTTPRIO1 : THTTPRIO;
   Context : SecurityService.OperationContext;
   Request : SecurityServiceRetrieveAppSecRequestDTO;
   Reply   : SecurityServiceRetrieveAppSecReplyCollectionDTO;
   RequiredAttributes : SecurityServiceRetrieveAppSecRequiredAttributesDTO;
   //RequiredAttributes : TableServiceReadRequiredAttributesDTO;
Begin
   HTTPRIO1 := THTTPRIO.Create(nil);
   HTTPRIO1.OnBeforeExecute := HTTPRIO2_BeforeExecute;

   Context := SecurityService.OperationContext.Create;
   Context.position := MimsPosition;
   Context.district := MimsDistrict;
   ...
   RequiredAttributes := SecurityServiceRetrieveAppSecRequiredAttributesDTO.Create;
   RequiredAttributes.returnAccessLevel := 'true';

   Request := SecurityServiceRetrieveAppSecRequestDTO.Create;
   //Request.appName := ApplicationName;
   Request.appName := 'SAQ301';
   Request.appType := 'M';
   Request.districtCode := MimsDistrict;

   try
    HTTPRIO1.WSDLLocation := Host +
      '/ews/services/vba/SecurityService?WSDL&op=retrieveAppSec';
     Reply := (HTTPRIO1 as Security).retrieveAppSec(Context, Request, RequiredAttributes, '');
     GetLevelAccess := reply.replyElements[0].accessLevel;
   except
...
var
   HTTPRIO1 : THTTPRIO;
   Context : TableCodeService.OperationContext;
   Request : TableCodeDTO;
   Reply   : TableCodeServiceResult;
   //RequiredAttributes : TableServiceReadRequiredAttributesDTO;
Begin
   HTTPRIO1 := THTTPRIO.Create(nil);

   Context := TableCodeService.OperationContext.Create;
   Context.position := MimsPosition;
   Context.district := MimsDistrict;

   Request := TableCodeDTO.Create;
   Request.tableType := ATable;
   Request.tableCode := ACode;

   HTTPRIO1.OnBeforeExecute := HTTPRIO1_BeforeExecute;
   HTTPRIO1.Port := 'TableServiceHttpPort';
   HTTPRIO1.Service := 'TableService';
   HTTPRIO1.Tag := 0;

   try
     HTTPRIO1.WSDLLocation := Host + '/ews/services/vba/TableCodeService?WSDL&op=read';
     Reply := (HTTPRIO1 as TableCode).read(Context, Request);
     GetTableValue := Reply.tableCodeDTO.tableDescription;
   except
    on E : Exception do begin
      GetTableValue := E.Message;
    end;
   end;
     {ExecProgram('MSO011');
...
procedure TMIMSXServer.HTTPRIO1_BeforeExecute(const MethodName:
  string; var SOAPRequest: WideString);
var
  StringToFind : String;
...
begin
  ...
  StringToFind := '<tableCode';
  StringToReplace := '<tableCode xmlns="http://instances.m3001.types.ellipse.mincom.com"';
  SOAPRequest := StringReplace(SOAPRequest, StringToFind, StringToReplace, [RfReplaceAll] );

  StringToFind := '<tableType';
  StringToReplace := '<tableType xmlns="http://instances.m3001.types.ellipse.mincom.com"';
  SOAPRequest := StringReplace(SOAPRequest, StringToFind, StringToReplace, [RfReplaceAll] );

  SetAuthenticationHeader( SOAPRequest );
end;

procedure TMIMSXServer.HTTPRIO2_BeforeExecute(const MethodName:
  String; var SOAPRequest: WideString);
var
  StringToFind : String;
  StringToReplace : String;
begin
  ...
  StringToFind := '<appName';
  StringToReplace := '<appName xmlns:sec="http://security.ellipse.enterpriseservice.mincom.com"';
  SOAPRequest := StringReplace(SOAPRequest, StringToFind, StringToReplace, [RfReplaceAll] );

  StringToFind := '<appType';
  StringToReplace := '<appType xmlns:sec="http://security.ellipse.enterpriseservice.mincom.com"';
  SOAPRequest := StringReplace(SOAPRequest, StringToFind, StringToReplace, [RfReplaceAll] );

  SetAuthenticationHeader( SOAPRequest );
end;

function TMIMSXServer.SetAuthenticationHeader(var SOAPRequest : WideString): String;
var
  StringToFind : String;
  StringToReplace : String;
...
begin
  try
        ...
        '<SOAP-ENV:Header> ' +
                '<wsse:Security xmlns:wsse="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd" SOAP-ENV:mustUnderstand="1"> ' +
                       ...
                       'xmlns:wsse="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd"> ' +
                                '<wsse:Username xmlns:wsse="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd">'+ MimsUser +'</wsse:Username>  ' +
    ...
    SOAPRequest := StringReplace(SOAPRequest, StringToFind, HeaderAuthentication, [RfReplaceAll] );
...
Begin
   ...
   frmSAQ00007ProfileConnection := TfrmSAQ00007ProfileConnection.Create(nil);
   frmSAQ00007ProfileConnection.ShowModal;

   try
     FUserName     := frmSAQ00007ProfileConnection.edtMimsUser.Text;
     FMimsUser     := frmSAQ00007ProfileConnection.edtMimsUser.Text;
     FMimsPassword := frmSAQ00007ProfileConnection.edtMimsPassword.Text;
     FHost         := frmSAQ00007ProfileConnection.edtHostName.Text;
     FPort         := frmSAQ00007ProfileConnection.edtHostName.Text;
     FEnviroment   := frmSAQ00007ProfileConnection.cbProfileName.Text;

     position := frmSAQ00007ProfileConnection.cbMimsPositions.Text;
     index := AnsiPos('-',position);
     if (index > 0 ) then
       position := Copy(position,1, index - 1);
     FMimsPosition := UpperCase(trim(position));

     district := frmSAQ00007ProfileConnection.cbMimsDistricts.Text;
     index := AnsiPos('-',district);
     if (index > 0 ) then
       district := Copy(district,1, index - 1);
     FMimsDistrict := UpperCase(trim(district));

     If Trim(FMimsUser) = '' Then Begin
        FMimsUser := FUserName;
     End;

     FMimsPassword := Trim(FMimsPassword);
     
     GetOracleUserPassword;
     //FOracleUserPassword := frmSAQ00007ProfileConnection.edtMimsPassword.Text;
   except
    on E : Exception do
...
procedure TMIMSXServer.HTTPRIO1BeforeExecute(const MethodName: String;
