...
Function AsPercent(AValue:Double):String;
Function AsMoney(AValue:Double):String;
...
Function GetMIMSKey2(AKey:String;ALength:Integer):String;
...
Procedure Nothing;
Procedure ClearTemporalTable(ATable:TTable);
implementation
Uses SysUtils,Windows,Forms;

Function AsPercent(AValue:Double):String;
Begin
     AsPercent := Format('%5.2f',[AValue])+'%';
End;
Function AsMoney(AValue:Double):String;
Begin
     AsMoney := Format('%m',[AValue]);
End;
Function VarIsUnAssigned(AVariant:Variant):Boolean;
Begin
...
Begin
     ...
     If Years = 0 Then Begin
        If FinalMonth = StartMonth Then Begin
           MonthDays := DayOfTheMonth(StartYear,StartMonth);
           If (StartDay = 1) And (FinalDay = MonthDays) Then Begin
              AMonths := 1;
              ADays   := 0; End
           Else Begin
              AMonths := 0;
              ADays   := FinalDay - StartDay + 1;
           End; End
        Else Begin
           If FinalDay >= StartDay Then Begin
              MonthDays := DayOfTheMonth(StartYear,FinalMonth);
              If (StartDay = 1) And (FinalDay = MonthDays) Then Begin
                 AMonths := FinalMonth - StartMonth + 1;
                 ADays   := 0; End
              Else Begin
           ...
           Else Begin
              ...
              AMonths := FinalMonth - StartMonth - 1;
              If (FinalDay + 1) = StartDay Then Begin
                 AMonths := AMonths + 1;
                 ADays   := 0; End
              Else Begin
                 ADays := FinalDay + (ADays - StartDay + 1);
              End;
           End;
        End; End
     Else Begin
        ...
        If FinalMonth >= StartMonth Then Begin
           If FinalDay >= StartDay Then Begin
              ...
              MonthDays := DayOfTheMonth(FinalYear,FinalMonth);
              If (StartDay = 1) And (FinalDay = MonthDays) Then Begin
                 AMonths := AMonths + 1;
                 ADays   := 0; End
              Else Begin
        ...
        Else Begin
           If FinalDay >= StartDay Then Begin
              ...
              If (StartDay = 1) And
                 (FinalDay = DayOfTheMonth(FinalYear,FinalMonth)) Then Begin
                 Inc(AMonths);
                 ADays := 0; End
              Else Begin
                 ADays   := FinalDay - StartDay + 1;
     ...
     If AnStartDate <= AFinalDate Then Begin
        ...
        If Months = 0 Then Begin
           If Days = 1 Then Begin
              GetMDTimeElapsed := Format('%d día',[Days]);End
           Else Begin
              GetMDTimeElapsed := Format('%d días',[Days]);
           End;End
        Else Begin
...
Begin
     ...
     Except
        ...
        On E:Exception Do
           begin
             AsString:=AKey;
             while length(AsString) < ALength do
             begin
               AsString:=Asstring+' ';
             end;
             GetMimsKey:=AsString;
...
Begin
     Try
        GetMIMSKey2 := GetKeyAsMIMSInteger(AKey,ALength);
     Except
        On E:Exception Do GetMIMSKey2 := AKey;
     End;
End;
